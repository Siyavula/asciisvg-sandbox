...
function drawPictures() { // main routine; called after webpage has loaded
 ...
 if (checkIfSVGavailable) {
  ...
  if (nd != null && notifyIfNoSVG && len>0)
    if (alertIfNoSVG)
      document.getElementById('error_msg').value = "To view the SVG pictures in Internet Explorer\n\
...
//    eval(src.replace(/\s\s/g,";")); //for XML version
    ...
    if (id == null || id == "") {
      ...
      picture.setAttribute("id",id);
		...
		{
			error_text = "Image FAILED to generate.\n"
			error_text += err
		}
...
function initPicture(x_min,x_max,y_min,y_max) {
 if (!initialized) {
  ...
  if (typeof xmin != "number" || typeof xmax != "number" || xmin >= xmax)
		document.getElementById('error_msg').value = "Picture requires at least two numbers: xmin < xmax";
 else if (y_max != null && (typeof y_min != "number" || 
          typeof y_max != "number" || y_min >= y_max))
   document.getElementById('error_msg').value = "initPicture(xmin,xmax,ymin,ymax) requires numbers ymin < ymax";
 else {
